...
// 获取队伍里每个人的一个formId
$teamQuery=PDOQuery($dbcon,'SELECT MAX(a.form_id) AS formId,a.open_id FROM wxmp_form_id a,wxmp_open_id b WHERE a.open_id=b.open_id AND b.team_id=? AND a.status=0 GROUP BY a.open_id',[$teamId],[PDO::PARAM_INT]);
...
foreach($teamList as $teamInfo){
	$req=curl('https://wx.itrclub.com/wisecity6/templateMessage.php?mod=send','post',['remark'=>'交易确认提醒-'.$orderId.'-'.$cnType,'accessToken'=>$accessToken,'templateId'=>'ApXZ8Hbnnwl-nxARQQ63uTLHpcXs9irJBsgJKc-3kFY','openId'=>$teamInfo['open_id'],'formId'=>$teamInfo['formId'],'data'=>$data,'page'=>'pages/function/index','emphasisKeyword'=>'keyword8.DATA','ip'=>getIP()]);
